...
n_bits = 4;
y_out=lappedQ(x, Fa, PQMF32_Hfilters, PQMF32_Gfilters, n_bits);
...
error = x(ordem+1:end-ordem)- y_out(1:end);

figure(5);
plot(ns(ordem+1:end-ordem)*Ta,error);
title('Erro de recuperação');

SNR_PQMF=snr(x(ordem+1:end-ordem), y_out(1:end),0)