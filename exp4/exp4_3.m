...
figure(3);
[H,W] = freqz(b0(num_frames/2+1), ai(:,num_frames/2+1),1024,2);
...
for m = 0:num_frames-2
    ...
    [z(m*Nframe+1:m*Nframe+Nframe), zi_y] = filter(ai_y(:,m+1), b0_y(m+1), y(m*Nframe+1:m*Nframe+Nframe), zi_y);
end
...
z((num_frames-1)*Nframe+1:end) = filter(ai_y(:,num_frames),b0_y(m+1), y((num_frames-1)*Nframe+1:end), zi_y);
